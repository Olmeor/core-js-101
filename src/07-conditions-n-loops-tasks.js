...
function getDigitalRoot(num) {
  ...
  const sum = arr.reduce((acc, e) => acc + e);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arrBrackets.length; i += 1) {
    if (brackets.includes(arrBrackets[i])) {
      brackets = brackets.replace(arrBrackets[i], '');
      i = -1;
    }
  }
  return !brackets;
...
function getCommonDirectoryPath(pathes) {
  const arrPath = pathes.map((e) => e.split('/'));
  arrPath.sort((a, b) => a.length - b.length);
  const firstPath = arrPath[0];
  arrPath.splice(0, 1);
  const res = [];
  for (let i = 0; i < firstPath.length; i += 1) {
    if (arrPath[0][i] === firstPath[i]) {
      res.push(firstPath[i]);
    }
  }
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < position.length; i += 1) {
    ...
    if ((position[0][0]) && (position[1][1] === position[0][0])
      && (position[2][2] === position[0][0])) {
      return position[0][0];
    }

    if ((position[0][2]) && (position[1][1] === position[0][2])
